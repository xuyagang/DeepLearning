...
> 用数学语言表达：给定由d个属性描述的示例   x = (x<sub>1</sub>,x<sub>2</sub>,x<sub>3</sub>,...x<sub>d</sub>)，表示x在第i个属性上的取值
>
> 线性模型就是==试图学习一个通过属性的线性组合来进行预测的函数==

​					__f(x)  = w1x1 + w2x2 + ... + wdxd +b__

用向量的形式来表达：

​					__f(x) = w<sup>T</sup>x + b__

其中 w = (w1,w2,...,wd),w和b都是需要学习的参数，模型通过不断调整w和b，最后得到最优模型

- 一维线性回归

  > 给定数据集D={(x1,y1),(x2,y2),(x3,y3)...(xm,ym)},线性回归希望优化出一个函数f(x)
  >
  > 使得   f(x<sub>i</sub>)  = wx<sub>i</sub> + b 能与 y<sub>i</sub> 尽可能接近
  >
  > - 如何学习到w和b,只需要确定如何衡量f(x) 和 y 之间的差别
  >
  >   - __Loss = $\sum^m_{i=1}(f(x_i) - y_i)^2$__来衡量误差，去平方是因为距离有正负，这就是均方误差
  >
  >   - 我们要找到$w^*和b*$，使得：
  >     $$
  >     (w^*,b^*) = arg min_{w,b}\sum^m_{i=1}(y_i-wx_i-b)
  >     $$
  >
  >     > 非常直观，对应了常用的欧几里得距离，基于均方误差最小化来进行模型求解的方式为“最小二乘法”
  >     >
  >     > - 求这个连续函数的最小值，只需要求它的偏导数,让偏导数等于0来估计它的参数
  >     >
  >     >   > 导数:是对含有一个自变量的函数进行求导。
  >     >   >
  ...
  >     >
  >     >   - $$
  >     >     \frac{\partial Loss(w,b)}{\partial w} = 2(w\sum _{i=1}^mx_i^2-\sum_{i=1}^m(y_i-b)x_i) = 0
  >     >     $$
  >     >
  >     >   - $$
  >     >     \frac{\partial Loss(w,b)}{\partial w} = 2(w\sum _{i=1}^mx_i^2-\sum_{i=1}^m(y_i-b)x_i) = 0
  ...
  >     >
  >     > - $$
  >     >   w = \frac{\sum_{i=1}^my_i(x_i-\overline{x})}{\sum^m_{i=1}-\frac{1}{m}(\sum^m_{i=1}x_i)^2}
  >     >   $$
  >     >
  >     > - $$
  ...
  >     >
  >     >   - $$
  >     >     \overline{x} = \frac{1}{m}\sum_{i=1}^m(y_i-wx_i)
  >     >     $$
  >     >
  >     >   - 
...
- 多维线性回归
  ...
  > 更一般的情况是多维线性回归，我们有d个属性，试图学得最优函数f(x):
  > $$
  > f(x_i) = w^Tx_i+b
  > $$
  > 
